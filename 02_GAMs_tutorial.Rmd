...
Now switching to LVDC3_Master_RawR_180208.csv ... only difference is that I added a column for date, which has the interpolation from Jack Cornett's original spreadsheet very approximate, still waiting on dates from Peter.

I didn't transfer over a lot of the exploratory stuff, like the PCAs, just so you know. 

I cleaned up and abbreviated the script on 2021/06/24 to share with Caitlin and Anna.
...
## Load functions
See Gavin's blog post for a detailed explanation of what all these functions do:
...
mod_del15N_S <- gamm(del15N ~ s(age_sky, k = 22),
...
del15NYear_S <- with(sky_wide,
                     data.frame(age_sky = seq(min(age_sky), max(age_sky), length.out = 200)))
del15NYear_S <- cbind(del15NYear_S,
                      data.frame(predict(mod_del15N_S$gam, del15NYear_S,
...
### type of predict()
del15NYear_L <- with(loch_wide,
                     data.frame(age_sky = seq(min(age_sky), max(age_sky),
                                              length.out = 200)))
del15NYear_L <- cbind(del15NYear_L,
                      data.frame(predict(mod_del15N_L$gam, del15NYear_L,
                                         type="response", se.fit = TRUE)))
### this calculates on the link scale (i.e., log)
...
###### > Periods of change
```{r}
#NOTE that we only calculated Periods of Change for Sky Pond because of issues with dating :(
```
...
del15N_plot
#Yay it works!
#Note that some axis labels are missing because in the end we'll have a large composite figure. 
...
#When is d13C decreasing?
decr<-data.frame(unlist(m2.dsig$decr), del13CYear_S$age_sky) %>%
  rename(value=unlist.m2.dsig.decr.,
         age_sky=del13CYear_S.age_sky)
decr

#When is d13C increasing?
incr<-data.frame(unlist(m2.dsig$incr), del13CYear_S$age_sky) %>%
  rename(value=unlist.m2.dsig.incr.,
         age_sky=del13CYear_S.age_sky)
incr
```
...
mod_del13C_L <- gamm(del13C ~ s(age_sky, k = 5), data = loch_wide,
                    correlation = corCAR1(form = ~ age_sky), method = "REML")
...
Diatoms as a proportion of total algal production
```{r}
...
bp_PRISM <- PRISM.confit[,1] #breakpoint 
bp_PRISM
CI.l_PRISM <- PRISM.confit[,2] #confidence interval lower
CI.l_PRISM
CI.u_PRISM <- PRISM.confit[,3] #confidence interval upper
CI.u_PRISM
