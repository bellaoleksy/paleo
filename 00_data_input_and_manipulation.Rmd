...
SPDC1_short <- SPDC1_raw %>%
  ...
  select(lake_ID:del15N, fuco, oscil, aphan, diadino, myxo, allo, diato,
...
LVDC3_short <- LVDC3_raw %>%
  mutate(chla_pheoa = chl_a + pheo_a,
         ...
         perc_chlb = (phaeo_b/pheo_a),
         perc_diatox = (diato/chla_pheoa)) %>%
  select(lake_ID, depth_plot:fuco, oscil, aphan, diadino, myxo, allo, diato, lutein, b_car, cantha, echine,
