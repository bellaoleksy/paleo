...
periodsChange1 <- periodsChange %>%
  mutate(proxy_ID=factor(proxy_ID,
                         levels= c("pheo_a", "phaeo_b", "diato"),
                         label = c("Pheophytin~italic(a)", "Pheophytin~italic(b)", "Diatoxanthin")))

periodsChange2 <- data.frame(proxy_ID=c("pheo_a", "phaeo_b", "diato"),
                            xmin=c(NA,1727.791,1914.283),
                            xmax=c(NA,1819,2013.610))
periodsChange2 <- periodsChange %>%
  mutate(proxy_ID=factor(proxy_ID,
                         levels= c("pheo_a", "phaeo_b", "diato"),
                         label = c("Pheophytin~italic(a)", "Pheophytin~italic(b)", "Diatoxanthin")))

tri_plot<-
  ggplot(bind_CI,aes(x=age_sky,y=fit, color=proxy_ID))+
  geom_rect(data = periodsChange, aes(xmin = xmin, xmax = xmax, ymin = -Inf, ymax = Inf),
            alpha = 0.1, fill="grey50", inherit.aes = FALSE)+
  geom_point(data=tri_long, mapping=aes(x=age_sky, y=response), size=2.5, alpha=0.7) +
  geom_line(size=1)+
  geom_ribbon(aes(ymin = (lower), ymax = (upper), x = age_sky, fill=proxy_ID), alpha = 0.5, inherit.aes = FALSE) +
  scale_y_continuous(breaks=seq(0, 1500, 300))+
  scale_x_continuous(breaks=seq(1600, 2000, 50))+
  labs(y="",x="Year")+
