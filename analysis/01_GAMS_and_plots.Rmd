...
I shared this tutorial with July on 2025/06/18
...
sky_wide <- read.csv(file="data/output/sky_wide.csv")
...
theme_paleo <- function () { 
  theme_base(base_size=12, base_family="SF Pro Display") %+replace% 
    theme(
...
incr<-data.frame(unlist(m2.dsig$incr), pheoaYear_S$age_sky)
```
...
incr<-data.frame(unlist(m2.dsig$incr), pheobYear_S$age_sky)
#### NO evidence of it increasing after 1973
...
specieswithdate <- read.csv("data/output/specieswithdates.csv")

B_P_separate<- specieswithdate %>%
  filter(age_sky > 1850) %>%
  ggplot(aes(x=age_sky,y=total, shape=PLANK_B, fill=PLANK_B, color=PLANK_B))+
  # annotate("rect", xmin=1850, xmax=2020, ymin=0, ymax=Inf, alpha=0.1, fill="grey10") +
  ...
  guides(shape = guide_legend(override.aes = list(size=3,linetype = 0)),
         color = guide_legend(override.aes = list(size=3,linetype = 0)))
B_P_separate
...
PB
 ggsave("figures/MS/Figure6_PB_alone_resubmission_FINAL_600dpi.png", width=4, height=6,units="in", dpi=600)
PB2 <- PB +
      theme(axis.text.x=element_text(size=24, angle = 45, vjust=0.55))  
...
B_P_stacked
ggsave("figures/MS/figure6_resubmission_PB.png", width=4, height=6,units="in",dpi=600)
...
### EXTRA - PRESENTATIONS
#### Pheon a, pheo b, diatox Sky Pond
```{r}
#Plot in MS form
unique(master_core_long$proxy_ID)
tri_long <- master_core_long %>% #sample_data_long
  filter(proxy_ID %in% c("pheo_a","phaeo_b","diato")) %>%
  filter(lake_ID == "SkyPond")%>%
  mutate(proxy_ID=factor(proxy_ID,
                         levels=c("pheo_a","phaeo_b","diato"),
                         label = c("Pheophytin~italic(a)", "Pheophytin~italic(b)", "Diatoxanthin")))

pheoaYear_S$proxy_ID <- "pheo_a"
pheobYear_S$proxy_ID <- "phaeo_b"
diatoxYear_S$proxy_ID <- "diato"
pheoaYear_S$proxy_ID_lab <- "pheo_a"
pheobYear_S$proxy_ID_lab <- "phaeo_b"
diatoxYear_S$proxy_ID_lab <- "diato"

bind_CI <- bind_rows(pheoaYear_S, pheobYear_S, diatoxYear_S) %>%
  mutate(proxy_ID=factor(proxy_ID,
                         levels=c("pheo_a","phaeo_b","diato"),
                         label = c("Pheophytin~italic(a)", "Pheophytin~italic(b)", "Diatoxanthin")))

# A data frame with labels for each facet
f_labels <- data.frame(proxy_ID = c("pheo_a", "phaeo_b", "diato"),
                       label = c("All algae", "Chlorophytes", "Diatoms"),
                       ypos = c(1500,1200,900))
f_labels <- f_labels %>%
  mutate(proxy_ID=factor(proxy_ID,
                         levels= c("pheo_a", "phaeo_b", "diato"),
                         label = c("Pheophytin~italic(a)", "Pheophytin~italic(b)", "Diatoxanthin")))

# A data frame for significant periods of change from GAMs 2nd derivative analysis
periodsChange1 <- data.frame(proxy_ID=c("pheo_a", "phaeo_b", "diato"),
                            xmin=c(1896.039,1904.148,1914.283),
                            xmax=c(2013.610,1977.123,2013.610))
periodsChange1 <- periodsChange1 %>%
  mutate(proxy_ID=factor(proxy_ID,
                         levels= c("pheo_a", "phaeo_b", "diato"),
                         label = c("Pheophytin~italic(a)", "Pheophytin~italic(b)", "Diatoxanthin")))

periodsChange2 <- data.frame(proxy_ID=c("pheo_a", "phaeo_b", "diato"),
                            xmin=c(NA,1727.791,1784.550),
                            xmax=c(NA,1819,1891))
periodsChange2 <- periodsChange2 %>%
  mutate(proxy_ID=factor(proxy_ID,
                         levels= c("pheo_a", "phaeo_b", "diato"),
                         label = c("Pheophytin~italic(a)", "Pheophytin~italic(b)", "Diatoxanthin")))

tri_plot<-
  ggplot(bind_CI,aes(x=age_sky,y=fit, color=proxy_ID, fill=proxy_ID))+
  geom_rect(data = periodsChange1, aes(xmin = xmin, xmax = xmax, ymin = -Inf, ymax = Inf),
            alpha = 0.3, fill="grey50", inherit.aes = FALSE)+
    # geom_rect(data = periodsChange2, aes(xmin = xmin, xmax = xmax, ymin = -Inf, ymax = Inf),
    #         alpha = 0.1, fill="grey50", inherit.aes = FALSE)+

    geom_line(size=1)+
  # geom_ribbon(aes(ymin = (lower), ymax = (upper), x = age_sky, fill=proxy_ID),
  #             alpha = 0.5, inherit.aes = FALSE,
  #             outline.type="both",
  #             linetype=0) +
  geom_line(aes(y=lower,color=proxy_ID), linetype="dashed") + 
  geom_line(aes(y=upper,color=proxy_ID), linetype="dashed") + 
    geom_point(data=tri_long, mapping=aes(x=age_sky, y=response), size=2.5, shape=21, color="#000000") +
  scale_y_continuous(breaks=seq(0, 1500, 300))+
  scale_x_continuous(breaks=seq(1600, 2000, 50))+
  labs(y="",x="Year")+
  geom_text(x = 1650, aes(y = ypos,label = label), data = f_labels)+
  scale_color_manual(values=c("#386641", "#a7c957","#d4a373"))+
  scale_fill_manual(values=c("#386641", "#a7c957","#d4a373"))+
  guides(shape = guide_legend(override.aes = list(size=10)),
         color = guide_legend(override.aes = list(linetype = 0)))+
  theme_pubr(base_size = 10)+
  facet_wrap(~proxy_ID,strip.position="right",
...
#Add y-axis label with special characters
tri_plot<-annotate_figure(tri_plot,
                left = text_grob(expression(Pigment~concentration~(nmol*~g^-1~organic~carbon)),
                                 color = "black", face="bold", size=12, rot = 90)
)
tri_plot
ggsave("figures/talks/top3_pigmentindicators.png", tri_plot,width=6, height=6,units="in", dpi=600)
